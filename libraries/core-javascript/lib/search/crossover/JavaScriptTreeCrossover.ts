...
import { prng, Crossover } from "@syntest/core";
...
export class JavaScriptTreeCrossover implements Crossover<JavaScriptTestCase> {
  ...
  constructor(crossoverProbability: number) {
  ...
  public crossOver(parents: JavaScriptTestCase[]): JavaScriptTestCase[] {
    ...
    while (queueA.length) {
      ...
      if (prng.nextBoolean(this.crossoverProbability)) {
        // crossover
  ...
  protected findSimilarSubtree(wanted: Statement, tree: Statement) {
    ...
    while (queue.length) {
      ...
      if (wanted.identifierDescription === pair.child.identifierDescription) {
