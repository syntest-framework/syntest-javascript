...
export class JavaScriptDecoder implements Decoder<JavaScriptTestCase, string> {
  ...
  getImport(sourceDir: string, dependency: Export): string {
    const _path = dependency.filePath.replace(
      ...
      path.join(sourceDir, path.basename(this.targetRootDirectory))
    );
  ...
  getLogDirectory(id: string, varName: string): string {
    return path.join(this.tempLogDirectory, id, varName);
  }
}
