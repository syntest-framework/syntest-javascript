...
export class ElementTypeMap {
  ...
  set(element: Element, typeProbability: TypeProbability) {
    const elementString = this.elementAsString(element);

    this.elementMap.set(elementString, element);
    this.typeMap.set(elementString, typeProbability);
  }

  has(element: Element): boolean {
    const elementString = this.elementAsString(element);

    return this.elementMap.has(elementString);
  }

  get(element: Element): TypeProbability {
    const elementString = this.elementAsString(element);

    return this.typeMap.get(elementString);
  }
