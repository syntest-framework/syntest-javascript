...
import { DiscoveredObjectKind, DiscoveredType } from "./DiscoveredType.js";
...
export class ObjectVisitor extends AbstractSyntaxTreeVisitor {
  ...
  public ObjectProperty: (path: NodePath<t.ObjectProperty>) => void = (
    ...
    if (path.get("key").isPrivateName()) {
      const name = this._getPropertyName(
        (<NodePath<t.PrivateName>>path.get("key")).get("id")
      );
  ...
  public MemberExpression: (path: NodePath<t.MemberExpression>) => void = (
    ...
    if (path.node.object.type === "ThisExpression") {
      ...
      if (path.get("property").isPrivateName()) {
        const name = this._getPropertyName(
          (<NodePath<t.PrivateName>>path.get("property")).get("id")
        );

        _object.properties.set(`#${name}`, this._getNodeId(path));
      } else {
        const name = this._getPropertyName(path.get("property"));
