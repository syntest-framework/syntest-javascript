...
export async function testSplitting(
  ...
  for (const [target, archive] of archives.entries()) {
    let encodings = archive.getEncodings();
    ...
    while (true) {
      const splitEncodings = await _testSplitting(runner, encodings);
      ...
      console.log("split found, repeating");
    }
...
async function _testSplitting(
  ...
  for (const encoding of originalEncodings) {
    ...
    const childEncodingPairs = _splitEncoding(encoding);
    ...
    for (const pair of childEncodingPairs) {
      // evaluate
      await runner.executeMultiple(pair);
      // are they equal?
      if (
        _equalResults(
          <JavaScriptExecutionResult>encoding.getExecutionResult(),
          pair.map(
 ...
 * @returns
 */
function _splitEncoding(encoding: JavaScriptTestCase): JavaScriptTestCase[][] {
...
function _equalResults(
  ...
  if (originalResult.hasError()) {
    ...
    for (const result of splitResults) {
      if (
        result.hasError() &&
        _equalErrors(originalResult.getError(), result.getError())
      ) {
        notEqual = false;
      }
    }
...
function _equalErrors(errorA: Error, errorB: Error) {
  return (
