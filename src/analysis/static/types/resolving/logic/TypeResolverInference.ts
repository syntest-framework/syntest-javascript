...
export class TypeResolverInference extends TypeResolver {
  ...
  resolveRelations(elements: Element[], relations: Relation[]): boolean {
    ...
    for (const relation of relations) {
      ...
      const involved: TypeProbability[] = relation.involved.map((e) =>
        this.getElementType(e)
      );
  ...
  resolveComplexElements(
    ...
    for (const element of elements) {
      ...
      let props = propertyAccessors.filter(
      ...
      props = props.filter((r) => {
        if (
          ...
          const propRelation = wrapperElementIsRelation.get(
            r.involved[0].value
          );
          if (propRelation.relation === RelationType.PropertyAccessor) {
            return (
              propRelation.involved[0].value === elRelation.involved[0].value &&
      ...
      for (const object of objects_) {
        ...
        for (const prop of properties) {
          if (
            object.properties.has(prop.value) ||
            object.functions.has(prop.value)
          ) {
            score += 1;
          }
        }
          ...
          object.properties.forEach((prop) => {
            if (object.propertyType && object.propertyType.has(prop)) {
              const typeMap = new TypeProbability([
                [object.propertyType.get(prop), 1, null],
              ]);
  ...
  getTyping(scope: Scope, variableName: string): TypeProbability {
    const elements = this.elements.filter((e) => !!e.scope);

    const correctFile = elements.filter(
      (e) => e.scope.filePath === scope.filePath
    );

    const correctVariable = correctFile.filter((e) => e.value === variableName);

    const correctScope = correctVariable.filter(
      (e) => `${e.scope.uid}` === `${scope.uid}`
    );
