...
export abstract class TypeResolver {
  ...
  setElementTypeToElement(
    ...
    if (typeElement.type === ElementType.Relation) {
      ...
      this.setElementType(
        relation,
        element,
        this.relationTyping.get(relation),
        value
      );
    } else {
      this.setElementType(
        relation,
        element,
        this.elementTyping.get(typeElement),
        value
      );
    }
  }
  ...
  setElementType(
    ...
    propertyTypings: Map<string, TypeProbability> = null
  ) {
  ...
  getElementType(element: Element) {
    if (element.type === "relation") {
      const relation: Relation = this.wrapperElementIsRelation.get(
        element.value
      );
